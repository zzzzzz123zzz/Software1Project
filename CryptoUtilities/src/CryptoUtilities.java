...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        if (m.isZero()) {
            ...
            // Store original n and compute remainder
            NaturalNumber originalN = new NaturalNumber2(n);

            // Calculation quotient q = originalN/m
            NaturalNumber q = new NaturalNumber2(originalN);
            q.divide(m); // q is now quotient

            //  Calculate the remainder r = originalN - (m * q)
            NaturalNumber mTimesQ = new NaturalNumber2(m);
            mTimesQ.multiply(q); // Calculate m * q
            NaturalNumber r = new NaturalNumber2(originalN);
            r.subtract(mTimesQ); // r = originalN - (m * q)

            // Recursive calculation of GCD(m, r)
            reduceToGCD(m, r);

            // After the recursion is over, update the result and clean m
            n.copyFrom(m);
            m.clear();
        }
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        boolean even = false;
        // Constant 2 for checking evenness
        NaturalNumber two = new NaturalNumber2(2);
        // Create a copy of n to avoid modifying original
        NaturalNumber nCopy = new NaturalNumber2();

        nCopy.copyFrom(n);

        // If n divided by 2 has no remainder, it's even
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p, NaturalNumber m) {
        ...
        NaturalNumber two = new NaturalNumber2(2);
        // Copy of p to avoid modifying original
        NaturalNumber pCopy = new NaturalNumber2(p);
        // Store original n value
        NaturalNumber originalN = new NaturalNumber2(n);

        // Case 1: p is even and not zero
        if (!pCopy.isZero() && isEven(pCopy)) {
            // if p is even and not 0, divide p by 2 to use fast recursive power
            pCopy.divide(two);
            powerMod(n, pCopy, m);
            ...
            // Case 2: p is odd and not zero
        } else if (!pCopy.isZero() && !isEven(pCopy)) {
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w, NaturalNumber n) {
        ...
        // Calculate n-1 for Fermat's theorem check
        power.copyFrom(n);
        power.decrement();

        // Copy w for two separate calculations
        temp1.copyFrom(w);
        temp2.copyFrom(w);

        // Calculate w^2 mod n and w^(n-1) mod n
        powerMod(temp1, two, n);
        powerMod(temp2, power, n);

        // Check if w is a witness (either w^2 = 1 mod n or w^(n-1) /= 1 mod n)
        if (temp1.equals(one) || !temp2.equals(one)) {
            isWitness = true;
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        // Initialize as prime until proven composite
        boolean isPrime = true;
        // Create n-1 for random number generation
        NaturalNumber nCopy = new NaturalNumber2(n);
        nCopy.decrement();
        NaturalNumber one = new NaturalNumber2(1);
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            for (int i = 1; i < FIFTY && isPrime; i++) {
                NaturalNumber w = randomNumber(nCopy);

                // Ensure w is in valid range (1 < w < n-1)
                while (w.compareTo(one) <= 0 || w.compareTo(nCopy) >= 0) {
                    w = randomNumber(nCopy);
                }
                // if it's composite, return false (is not prime)
                if (isWitnessToCompositeness(w, n)) {
                    isPrime = false;
                }
            }
        }
        // if it doesn't return false, it's probably prime
        return isPrime;
    }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        NaturalNumber newPrime = new NaturalNumber2();
        newPrime.copyFrom(n);
