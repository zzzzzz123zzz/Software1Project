...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String combination;
        // Combine str1 and str2 with overlap removed from str1
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet, String str) {
        ...
        // Check if str is a substring of any existing element
        boolean isSubstring = false;
        for (String s : strSet) {
            // Once isSbustring is true, we are done,
            // str is a substring of existing element
            if (s.contains(str) && !isSubstring) {
                isSubstring = true;
            }
        }
        if (!isSubstring) {
            // Collect elements that are str's substrings
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        // Process each line from input and add to the set while avoiding substrings
        while (!input.atEOS()) {
            String line = input.nextLine();
            // Use the method addToSetAvoidingSubstrings
            addToSetAvoidingSubstrings(inputStr, line);
        }
        return inputStr;
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        // Temporary variable to hold the input text
        String tempText = text;

        // Iterate through each character in the text
        for (int i = 0; i < tempText.length(); i++) {
            if (text.charAt(i) == '~') {
                // Print a newline when encountering '~'
                out.print("\n");
            } else {
